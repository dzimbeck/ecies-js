...
ecies-js is a pure JavaScript implementation of the Elliptic Curve Integrated Encryption Scheme (ECIES), offering simple encryption and decryption with compatibility for Bitcoin and Ethereum-style key pairs. Encrypt with a public key and decrypt with the matching private key—under the hood, a temporary key pair and elliptic-curve Diffie–Hellman derive a shared secret to encrypt the message.
